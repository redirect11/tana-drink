...
IL BADGE CONTA QUELLO CHE SI VEDE ── «Non mi e' chiaro come conta i filtri. Secondo me non funziona» (l'utente, con lo screenshot di una lavagna a sei colonne e un «▾ Filtri ①»). Il numero era `filtriAccesi.length`, cioe' quanti GENERI di filtro stringevano: e le colonne del banco, per quante fossero, erano un genere solo. Adesso conta le DEVIAZIONI una per una — tre colonne diverse dal normale fanno 3, con lo staff filtrato 4 — e chi guarda puo' contare a occhio e ritrovare lo stesso numero (BUG-080).

NON CAMBIA COSA SI CONTA: sempre le deviazioni dal NORMALE, non i filtri accesi. Al primo avvio, tutto di serie, il badge e' spento — e' la ragione per cui `corsieDiverseDalNormale` esiste (BUG-058). E a fila APERTA resta zero: i chip si vedono da se'. Quella regola sta adesso in `contaFiltri(accesi, aperti)`, non in un ternario della pagina, ed e' provata a unita' come il suo gemello `spiegaFiltri`.

UNA LISTA SOLA PER IL BADGE E PER IL TITLE. Una voce e' un nome secco («Chiusi», «Solo oggi») e vale uno, oppure un gruppo che porta quante deviazioni tiene dentro (`{ nome: 'Colonne', quante: 3 }`): il badge somma, il title raggruppa. Due liste, o un numero calcolato per conto suo, sarebbero due verita' sulla stessa cosa. «SOLO OGGI»

VALE IN TUTTE LE VISTE, e prima veniva contato solo in griglia. Taglia `ordersInVista`, la lista da cui scendono tutte e quattro le viste: chi lo accendeva in griglia e poi passava alle comande si portava dietro il taglio senza niente a schermo che lo dicesse. Adesso il chip e' uno solo (`chipSoloOggi`), sta in tutte le file — fuori dalla griglia solo se e' ACCESO, che li' e' l'unico modo per spegnerlo — e il badge lo conta ovunque.

**Dove**: `src/pages/BartenderPage.jsx (filaFiltri, pastigliaFiltri, chipSoloOggi), src/lib/coda.js (etichettaFiltri, spiegaFiltri, contaFiltri, SCHEDE_GRIGLIA), src/lib/impostazioniLocali.js (filtriAperti), src/index.css` · **Lo dimostrano**: `tests/unit/coda.test.js`, `tests/unit/css.test.js`, `tests/component/CodaCorsie.test.jsx`, `tests/component/CodaGiornate.test.jsx`
