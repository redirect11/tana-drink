...
// CHI SI PUÒ SCEGLIERE: gli admin del locale, quello collegato per primo.
//
// «Associati» vuol dire questo: sono gli admin dello stesso locale, e non
// c'è un elenco da compilare a mano da nessuna parte. Un locale con tre
// admin ha già detto tutto nominandoli; una lista di associazioni in più
// sarebbe una schermata da tenere allineata a quella dei ruoli, e due
// elenchi della stessa cosa divergono sempre.
//
// CHI È COLLEGATO STA IN CIMA perché è la risposta giusta quasi sempre: chi
// apre l'app col proprio account di solito è quello che ci lavora, e il
// primo della lista è quello che si tocca senza pensarci.
export function operatoriSelezionabili(staff, uidCollegato) {
  return (staff || [])
...
export function operatoreCorrente(staff, uidCollegato) {
  const ammessi = operatoriSelezionabili(staff, uidCollegato)
...
export function valeLaPenaChiedere(staff, uidCollegato) {
  return operatoriSelezionabili(staff, uidCollegato).length > 1
}
